...
- 矩形が小さすぎるとGrabCutが失敗します。エラーメッセージが出た場合は矩形を広めに指定してください
- 背景と前景のコントラストが低い場合は精度が下がります。手動で仕上げてください
...
│  └─ grabcut_tool.py             GrabCut半自動マスク生成（v0.4A追加）
└─ ui/
