...
| **大画像を縮小して処理する** | **ROI切り出し＋縮小でGrabCutを実行（v0.4A.1）** | **ON** |
| **GrabCut最大処理サイズ** | **ROI長辺の上限（px）。超えた場合に縮小する（v0.4A.1）** | **2048** |
...
│  ├─ grabcut_tool.py             GrabCut最適化処理・入力検証（v0.4A.1更新）
│  └─ grabcut_worker.py           GrabCutバックグラウンドWorker（v0.4A.1追加）
├─ ui/
│  ├─ main_window.py              メインウィンドウ・操作統括
│  ├─ image_canvas.py             キャンバス・各種編集モード・差分表示・ズーム・パン
│  └─ image_list_panel.py         画像一覧・フィルタ・ステータス表示
└─ tests/                         自動テスト（v0.4A.1追加）
   ├─ test_grabcut_tool.py        GrabCutツール単体テスト
   ├─ test_grabcut_large_image.py 大画像処理テスト
   ├─ test_grabcut_worker.py      Workerシグナルテスト
...
tests/test_grabcut_worker.py        - シグナル / キャンセル / 例外処理
